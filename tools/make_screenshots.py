...
产物输出到 screenshots/ 目录：
    01_start.png        开始界面
    02_level_select.png 关卡选择界面
    03_playing.png      游戏界面
    04_collision.png    碰撞反馈
    05_win.png          通关界面
    06_fail.png         失败界面
...
from game.app import App, FailScene, GameScene, LevelSelectScene, StartScene, WinScene
...
def main():
    ...
    # 2. 关卡选择界面（模拟已通关前 3 关，第 4 关进行中）
    app.save_data["unlocked"] = 4
    app.save_data["stars"] = {"0": 3, "1": 2, "2": 3}
    app.save_data["endless_best"] = 1280
    app.scene = LevelSelectScene(app)
    snapshot(app, "02_level_select.png")

    # 3. 游戏界面（第二关）
    app.scene = GameScene(app, 1)
    app.scene.update(12.3)  # 模拟已游戏 12.3 秒
    snapshot(app, "03_playing.png")

    # 4. 碰撞反馈：点击第二关被阻挡的 (0,2)↓（被 (3,2) 阻挡）
    app.scene = gs = GameScene(app, 1)
    gs._click_board(gs.cell_rect(0, 2).center)
    gs.update(0.12)
    snapshot(app, "04_collision.png")

    # 5. 通关界面
    app.scene = WinScene(app, 1, mistakes_left=2, score=1260, elapsed=21.4)
    app.scene.update(0.5)
    snapshot(app, "05_win.png")

    # 6. 失败界面
    app.scene = FailScene(app, 2)
    snapshot(app, "06_fail.png")

    # 7. 演示 GIF：按求解顺序自动通关第一关
    make_demo_gif(app)
    print("done.")
